...
const taskJoiSchema = Joi.object({
	...
	category: Joi.string().required().valid('to-do', 'in progress', 'done').default('to-do').messages({
	...
	.custom((value, helper) => {
		...
		if (!isValid(parsedStart)) {
			return helper.message(`Invalid start time format ${start}. Use HH:mm (e.g., '09:00').`);
		}

		const parsedEnd = parse(end, dateFormats.day, new Date());
		if (!isValid(parsedEnd)) {
			return helper.message(`Invalid start time format ${end}. Use HH:mm (e.g., '09:00').`);
		}
		if (parsedEnd <= parsedStart) {
			return helper.message(`End time ${end} must be later than start time ${start}.`);
		}

		const parsedDate = parse(date, dateFormats.year, new Date());
		if (!isValid(parsedDate)) {
			return helper.message(`Invalid date format ${date}.  Use yyyy-MM-dd (e.g., '2023-01-01').`);
		}
	})
	.messages({
